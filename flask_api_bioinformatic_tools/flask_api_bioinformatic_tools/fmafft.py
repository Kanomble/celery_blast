...
def perform_fasttree_phylobuild(project_id, query_sequence_id):
    if request.method == 'POST':
        ...
        return Response("1", status=HTTPStatus.BAD_REQUEST, mimetype="str")
#http://tools:5001/perform_fasttree_phylobuild_with_all_qseqs/" + str(project_id)
@fmafft.route('/perform_fasttree_phylobuild_with_all_qseqs/<int:project_id>',methods=['POST'])
def perform_fasttree_phylobuild_with_all_qseqs(project_id):
    if request.method == 'POST':
        ...
        for qseqid in query_sequence_ids:
            path_to_query_file = path_to_project + qseqid + '/target_sequences.msa'
            output = path_to_project + qseqid + '/target_sequences.nwk'

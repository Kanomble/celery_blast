...
with open(snakemake.log['log'],'w') as logfile:
    try:
        ...
        if len(treefile_content) > 0:
            ...
            hits = findall('[(,]{1}([a-zA-Z_0-9]+[a-zA-Z_0-9._]+):{1}',treefile_content[0])
            ...
            for name, sacc in zip(scientific_names['sscinames'], scientific_names['sacc']):
                if type(name) == str:
                    ...
                    logfile.write("\tWARNING:filling {}, {} with NaN\n".format(name,sacc))
            ...
            for hit in hit_to_name.keys():
                treefile_content[0] = sub(hit, hit_to_name[hit], treefile_content[0])
